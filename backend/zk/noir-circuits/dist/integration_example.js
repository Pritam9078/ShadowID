...
async function generateAgeProof(inputs) {
 ...
 */
async function generateCitizenshipProof(inputs) {
 ...
 */
async function generateAttributeProof(inputs) {
 ...
 */
async function performKYCVerification(userAddress, shadowIdRegistryContract // ethers.Contract
) {
    ...
    await shadowIdRegistryContract.submit_kyc(userAddress, ageProof.commitment);
    console.log("✅ Age KYC commitment submitted");
    // Submit citizenship proof hash  
    await shadowIdRegistryContract.submit_proof(userAddress, citizenshipProof.proofHash);
    console.log("✅ Citizenship proof hash submitted");
    // Submit attribute proof hash
    await shadowIdRegistryContract.submit_proof(userAddress, attributeProof.proofHash);
 ...
 */
async function checkVerificationStatus(userAddress, shadowIdRegistryContract) {
    return await shadowIdRegistryContract.is_verified(userAddress);
}
/**
 * DAO voting with ZK verification
 */
async function voteOnProposal(proposalId, vote, // 0 = against, 1 = for, 2 = abstain
daoContract, proofs) {
    ...
    await daoContract.vote(proposalId, vote, proofs.ageProof.commitment, proofs.citizenshipProof.proofHash);
