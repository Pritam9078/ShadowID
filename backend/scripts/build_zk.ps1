...
if (Test-Path "Nargo.toml") {
    ...
    try {
        ...
        Write-Error "Workspace compilation failed: $($_.Exception.Message)"
        exit 1
    }
} else {
...
Write-Info "Running circuit tests..."

# Run tests if available
try {
    nargo test --workspace
    if ($LASTEXITCODE -eq 0) {
        Write-Success "All circuit tests passed"
    } else {
        Write-Warning "Circuit tests failed or not available"
    }
} catch {
    Write-Warning "Circuit tests failed or not available"
}

Write-Info "Checking Aztec backend integration..."

# Check Aztec backend
Set-Location "zk\scripts"
try {
    node aztec_backend.js check
    if ($LASTEXITCODE -eq 0) {
        Write-Success "Aztec backend integration ready"
    } else {
        Write-Warning "Aztec backend not available, using nargo fallback"
    }
} catch {
    Write-Warning "Aztec backend check failed"
}
Set-Location "..\.."
...
Write-Host "  Verify proof:    .\zk\scripts\verify_proof.ps1 [circuit_name]"
...
Write-Host "  npm run zk:prove:win   - Generate proofs (interactive)"
