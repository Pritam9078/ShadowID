...
const UserProfile = ({ userId, isOwnProfile = false }) => {
  ...
  useEffect(() => {
    if (isOwnProfile && !address) {
      // If it's own profile but no wallet connected, show connection message
      setLoading(false);
      return;
    }
  ...
  const loadUserProfile = async () => {
    try {
      ...
      if (isOwnProfile && address) {
        // Try to get current user's profile by address
        try {
          profileData = await backendAPI.getUserProfile(address);
        } catch (apiError) {
          // If API fails, create a mock profile for development
          if (apiError.message?.includes('User not found') || apiError.message?.includes('No token')) {
            console.log('Creating fallback profile data for development');
            profileData = {
              address: address,
              username: null,
              email: null,
            ...
            setProfile(null); // Show registration form
            return;
          }
          throw apiError;
        }
      } else if (userId) {
        profileData = await backendAPI.getUserProfile(userId);
      } else {
      ...
      // If user not found and this is own profile, show registration form
      if (isOwnProfile && (error.message?.includes('User not found') || error.message?.includes('No token'))) {
        setProfile(null); // This will trigger the registration form
      } else {
  ...
  const loadVotingHistory = async () => {
    try {
      ...
      try {
        ...
        // For development, create some mock voting history
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      try {
        ...
        // For development, update the local profile if API fails
        console.log('API update failed, updating local profile for development');
        setProfile({ ...profile, ...formData });
        setIsEditing(false);
  ...
  const handleRegisterUser = async () => {
    try {
      ...
      try {
        ...
        // For development, create a mock profile if API fails
        console.log('API registration failed, creating mock profile for development');
        const mockProfile = {
          address: address,
          username: formData.username,
          email: formData.email,
        ...
        toast.success('Profile created successfully (Development Mode)');
      }
    } catch (error) {
